...
const Boombox = () => {
  ...
  const [sel, setSel] = useState({ value: curSong, label: curSong !== null ? `${songs[curSong].name} - ${songs[curSong].artists}` : '' });
  ...
  useEffect(() => {
    if (sel.value !== null) {
      ...
      dispatch(getLrc({
        spotifyTrackId: songs[sel.value].spotifyTrackId,
  ...
  Object.keys(songs).forEach((i) => {
    options.find((group) => group.label === songs[i].difficulty).options.push({ label: `${songs[i].name} - ${songs[i].artists}`, value: Number(i) })
  });
